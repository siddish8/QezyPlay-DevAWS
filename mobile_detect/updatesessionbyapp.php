...
//REMOVE BELOW (only for testing) LOCAL dbcon Added on 27-04-2016
	...
	$session=explode("-",$session_id);
	if($user_id > 0){

		//user identified
		$sql1 = "SELECT id,start_datetime FROM visitors_info WHERE user_id = ? AND page_id = ? AND ip_address = ? AND device = ? AND os_name = ? AND os_version = ? AND browser_name = ? AND browser_version = ? AND SUBSTRING_INDEX(session_id,'-',1) = ? AND play = ?";
		$data1 = array($user_id,$page_id,$ip_address,$device,$os_name,$os_version,$browser_name,$browser_version,$session[0],$play);
		$stmt1 = $dbcon->prepare($sql1, array(PDO::ATTR_CURSOR => PDO::CURSOR_SCROLL));
		$stmt1->execute($data1);
		...
		//echo "Row_id:".$result['id'];
		if($stmt1->rowCount() > 0){
			echo "already got infi, so just update";
			$result = $stmt1->fetch();
			...
			echo "no info so insert first time";
		...
		$sql1 = "SELECT id,start_datetime FROM visitors_info WHERE page_id = ? AND ip_address = ? AND device = ? AND os_name = ? AND os_version = ? AND browser_name = ? AND browser_version = ? AND SUBSTRING_INDEX(session_id,'-',1) = ? AND play = 0";
		$data1 = array($page_id,$ip_address,$device,$os_name,$os_version,$browser_name,$browser_version,$session[0]);
		//$sql1 = "SELECT id,start_datetime FROM visitors_info WHERE page_id = ? AND ip_address = ? AND device = ? AND os_name = ? AND os_version = ? AND browser_name = ? AND browser_version = ? AND play = 0";
		//$data1 = array($page_id,$ip_address,$device,$os_name,$os_version,$browser_name,$browser_version);
		$stmt1 = $dbcon->prepare($sql1, array(PDO::ATTR_CURSOR => PDO::CURSOR_SCROLL));
		$stmt1->execute($data1);	
