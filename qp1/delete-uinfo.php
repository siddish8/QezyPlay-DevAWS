...
if(($_POST['v_userid'])!=""){
	...
	echo "User-id: ".$v_userid=$_POST['v_userid']."\n";

	if($v_userid!=""){
		echo "Visitors Info-Count:".$count=get_var("SELECT count(*) from visitors_info where user_id=".$v_userid."");
		if($count>0){
			$sql1 = "DELETE FROM visitors_info WHERE user_id = ".$v_userid."";
			$r1=execute($sql1);
			if($r1)
			{
				echo "\n"."Deleted";
			$msg.="Deleted from visitors info for $v_userid <br>";
			}
...
$usmail=get_var("SELECT user_email from wp_users WHERE ID='".$usrid."'");
$msg.="<br> UserID: ".$usrid;
$msg.="<br> UserName: ".$usname;
$msg.="<br> UserEmail: ".$usmail;
}

if(($_POST['get_uid'])!=""){	

$uname=$_POST['uname'];	
$usid=get_var("SELECT ID from wp_users WHERE user_login='".$uname."' or user_email='".$uname."' ");
$Xusname=get_var("SELECT user_login from wp_users WHERE ID='".$usid."'");
$Xusmail=get_var("SELECT user_email from wp_users WHERE ID='".$usid."'");
$msg.="<br> UserID: ".$usid;
$msg.="<br> UserName: ".$Xusname;
$msg.="<br> UserEmail: ".$Xusmail;
}

if(isset($_POST['reg'])){

//Registrations Info Delete
$msg.="<br> UserID:".$id=$_POST['userid'];
$Yusname=get_var("SELECT user_login from wp_users WHERE ID='".$id."'");
$Yusmail=get_var("SELECT user_email from wp_users WHERE ID='".$id."'");
$msg.="<br> UserName: ".$Yusname;
$msg.="<br> UserEmail: ".$Yusmail;
$msg.="<br>";
...
$msg.="<br> UserID:".$id=$_POST['userid'];
$Zusname=get_var("SELECT user_login from wp_users WHERE ID='".$id."'");
$Zusmail=get_var("SELECT user_email from wp_users WHERE ID='".$id."'");
$msg.="<br> UserName: ".$Zusname;
$msg.="<br> UserEmail: ".$Zusmail;
$msg.="<br>";
...
$msg.="<br> UserID:".$id=$_POST['userid'];
$msg.="<br> UserName: ".$name=$_POST['username'];
$zusmail=get_var("SELECT user_email from wp_users WHERE ID='".$id."'");
$msg.="<br> UserEmail: ".$zusmail;
...
$msg.="<br> UserID:".$id=$_POST['userid'];
$msg.="<br> UserName: ".$name=$_POST['username'];
$Ausmail=get_var("SELECT user_email from wp_users WHERE ID='".$id."'");
$msg.="<br> UserEmail: ".$Ausmail;
...
$sql12 = "DELETE FROM visitors_info WHERE user_id = ".$id."";
execute($sql12);

$sql1 = "DELETE FROM wp_users WHERE ID = ".$id."";
execute($sql1);

$msg.="<br> Deleted All info includes: Reg, Sub, Analytics";
}
...
<input type="hidden" name="username" id="username" value="'.$g.'" />
					...
					<div class="4u 12u(medium)">
						<form method="post">
						<input type="text" name="v_userid" id="v_user_id"><br>
						<input type="submit" name="v_delete_info" value="Delete from Visitors info">
						</form>
