...
if ($page > 1)
 ...
 $mobileselected =  $pcselected = $androidselected =  $iosselected = $internalusers =  $valuableusers = $cond = $filter = "";
if($_REQUEST['users'] == "internal"){
	$cond .= " AND user_id in (SELECT user_id from user_qezy_testlist)";
	$internalusers = "selected='selected'";
	$searchstring .= "&users=internal";
	$filter.=" Users : Internal | ";
}elseif($_REQUEST['users'] == "valuable"){
	$cond .= " AND user_id not in (SELECT user_id from user_qezy_testlist)";
	$valuableusers = "selected='selected'";
	$searchstring .= "&users=valuable";
	$filter.=" Users : Valuable* | ";
}
...
if($_REQUEST['device'] == "pc"){
	...
	if($_REQUEST['device_type'] == "QezyPlayAndroid"){

	$cond .= " AND browser_name = 'QezyPlayAndroid'";
	$androidmobile = "selected='selected'";
	$searchstring .= "&device_type=QezyPlayAndroid";
	$filter.=" Device-type : Android | ";

	}elseif($_REQUEST['device_type'] == "QezyPlayiOS"){

	$cond .= " AND browser_name = 'QezyPlayiOS'";
	$iosmobile = "selected='selected'";
	$searchstring .= "&device_type=QezyPlayiOS";
	$filter.=" Device-type : iOS | ";
...
if($_REQUEST['channel_id'] != ""){
	...
	$cond .= " AND page_id = '".$_REQUEST['channel_id']."'";
	$searchstring .= "&channel_id=".$_REQUEST['channel_id'];

	$filter.=" Channel : ".$_REQUEST['channel_id']." | ";
...
$channels=get_all('SELECT * from channels');
					...
					<div class="card-block ">
                                    <form role="form" id="xoouserultra-login-form-1" method='post' enctype="multipart/form-data">
										...
										<div class="form-inline"> 
											...
											<select class="form-control underlined" id="users" name="users" onchange="">
												<option value=''>All Users</option>
												<option value='internal' <?php echo $internalusers; ?>>Internal Users</option>
												<option value='valuable' <?php echo $valuableusers; ?>>Valuable Users*</option>
											</select>	
										</div>
					
										<div class="form-inline"> 
											<label class="control-label">Device</label> &nbsp; 
											<select class="form-control underlined" id="device" name="device" onchange="showDeviceType()">
										...
										<div class="form-inline" id="d_type" style="display:none" > 
											<label class="control-label">Device Type</label> &nbsp; 
											<select class="form-control underlined" name="device_type">
												<option value=''>All</option>
												<option value='QezyPlayAndroid' <?php echo $androidmobile; ?>>Android</option>
												<option value='QezyPlayiOS' <?php echo $iosmobile; ?>>iOS</option>
											</select>	
										</div>
									
										<div class="form-inline"> 
											<label class="control-label">Channel</label> &nbsp; 
											<select class="form-control underlined"  name="channel_id" name="channel_id">
												<option value=''>All</option>
												<?php
												foreach($channels as $channel){
													$channelselected = ($channel['id'] == $_REQUEST['channel_id']) ? 'selected="selected" ' : "";
													echo "<option ".$channelselected."value='".$channel['id']."'>".$channel['name']."</option>";
												}
												?>
											</select>
										</div>
...
<section class="section">
		...
		<input type="hidden" name="device_type" value="<?php echo $_REQUEST['device_type']; ?>" >
...
function showDeviceType(){
	...
	var mobile=jQuery("#device").val();
	//alert(mobile);
	if(mobile=="mobile"){
		jQuery("#d_type").show();
		
	}else{
		jQuery("#device_type").val("");
		jQuery("#d_type").hide();
