...
<br />
<h4>All-time KNOWN USERS for your channel</h4>
<h3><?php echo $all_users = get_var("SELECT count(*) as count FROM(SELECT * FROM visitors_info where user_id>0 " . $cond . " group by user_id order by id desc) a") ?></h3>
<br />
<h4>Active or Live Time KNOWN USERS for your channel</h4>
<h3><?php echo $all_users = get_var("SELECT count(*) as count FROM(SELECT * FROM visitors_info where user_id>0 " . $cond . "" . $condition . " group by user_id order by id desc) a") ?></h3>
</div>
...
if ($play_hits > 0)
	{
	$sql1 = "select a.page_id,b.post_title,count(*) as hits from visitors_info a inner join wp_posts b on b.ID=a.page_id where b.post_status='publish' and b.post_type='post' " . $condA . " group by a.page_id order by hits desc";
	$res1 = get_all($sql1);
...
if ($play_hits > 0)
	{
	$sql1 = "select a.page_id,b.post_title,count(*) as hits from visitors_info a inner join wp_posts b on b.ID=a.page_id where b.post_status='publish' and b.post_type='post' " . $condA . "" . $condition . " group by a.page_id order by hits desc";
	$res1 = get_all($sql1);
...
$non_play_hits = get_var("SELECT count(*) as hits FROM visitors_info where user_id>0 and play=0 " . $cond . "" . $condition . " ");
$total_channel_page_hits = get_var("SELECT count(*) as hits FROM visitors_info where user_id>0 " . $cond . "" . $condition . " ");
echo "<h3>Channel Hits:" . $total_channel_page_hits . "(" . $play_hits . "+" . $non_play_hits . ")</h3>";
$total_all_page_hits = get_var("SELECT count(*) as hits FROM visitors_info" . $wherecond . "" . $cond . "");
echo "<h3>All Page Hits:" . $total_all_page_hits . "</h3>";
echo "</div>";
...
if ($play_duration > 0)
	{
	$sql2 = "select a.page_id,b.post_title,sum(duration) as duration from visitors_info a inner join wp_posts b on b.ID=a.page_id where b.post_status='publish' and b.post_type='post' " . $condA . " group by a.page_id order by duration desc";
	$res2=get_all($sql2);
...
$nonplay_duration = get_var("SELECT sum(duration) as duration FROM visitors_info where user_id>0 and play=0 " . $cond . "");
$total_duration = get_var("SELECT sum(duration) as duration FROM visitors_info where user_id>0 " . $cond . "");
...
$all_page_duration = get_var("SELECT sum(duration) as duration FROM visitors_info " . $cond . "");
...
$play_duration = get_var("SELECT sum(duration) as duration FROM visitors_info where user_id>0 and play=1" . $condition . " " . $cond . "");

if ($play_duration > 0)
	{
	$sql2 = "select a.page_id,b.post_title,sum(duration) as duration from visitors_info a inner join wp_posts b on b.ID=a.page_id where  b.post_status='publish' and b.post_type='post' " . $condA . "" . $condition . " group by a.page_id order by duration desc";
	$res2 = get_all($sql2);
...
$nonplay_duration = get_var("SELECT sum(duration) as duration FROM visitors_info where user_id>0 and play=0 " . $cond . " " . $condition . " ");
$total_duration = get_var("SELECT sum(duration) as duration FROM visitors_info where user_id>0 " . $cond . " " . $condition . " ");
echo "<h3>Channel Viewing duration:" . min_hr($total_duration)[0].min_hr($total_duration)[1]. "(" . min_hr($play_duration)[0].min_hr($play_duration)[1]. "+" . min_hr($nonplay_duration)[0].min_hr($nonplay_duration)[1] . ")</h3>";

$all_page_duration = get_var("SELECT sum(duration) as duration FROM visitors_info " . $cond . " " . $wherecond . " ");
echo "<h3>All Page Viewing duration:" . min_hr($all_page_duration)[0].min_hr($all_page_duration)[1] . "</h3>";
echo "</div>";
//echo "</div>";
...
if (isset($_POST['alphaS']))
	...
	$sql = "SELECT * from wp_users where user_login like '" . $char . "%' " . $cond . "";
	$users = get_all($sql);
...
if (isset($_POST['alphaU']))
	{
	$user = $_POST['alphaU'];
	$sql = "SELECT id from wp_users where user_login='" . $user . "' " . $cond . "";
	$user = get_var($sql);
...
$sql6 = "select a.user_id,a.user_name,sum(duration) as duration from visitors_info a inner join wp_posts b on b.ID=a.page_id where a.user_id>0 and a.play=1 and b.post_status='publish' and b.post_type='post' " . $alphaCondA . $alphaXCondA . "" . $condA . " group by a.user_name order by duration desc";
$res6 = get_all($sql6);
...
foreach($res6 as $r6)
	...
	$sql61 = "select a.page_id,b.post_title,sum(duration) as duration from visitors_info a inner join wp_posts b on b.ID=a.page_id where b.post_status='publish' and b.post_type='post' and user_id=" . $r6['user_id'] . " and a.play=1 and duration>0 " . $condA . " group by a.page_id order by duration desc";
	$res61 = get_all($sql61);
	$durationInfo = "";
...
echo "</tbody></table>";
$sql7 = "select a.user_id,a.user_name,sum(duration) as duration from visitors_info a inner join wp_posts b on b.ID=a.page_id " . $alphaCondA . $alphaXCondA . " " . $condA . " group by a.user_name order by duration desc";
$res7 = get_all($sql7);
...
$sql9 = "select a.user_id,a.user_name,count(*) as hits from visitors_info a inner join wp_posts b on b.ID=a.page_id where a.user_id>0 and a.play=1 and b.post_status='publish' and b.post_type='post' " . $alphaCondA . $alphaXCondA . " " . $condA . "  group by a.user_name order by hits desc";
$res9 = get_all($sql9);
//echo "<div style='display:inline-flex'>";
...
foreach($res9 as $r9)
	...
	$sql91 = "select a.user_name,a.page_id,b.post_title,count(*) as hits from visitors_info a inner join wp_posts b on b.ID=a.page_id where a.user_id>0 and b.post_status='publish' and b.post_type='post' and a.play=1 and user_id=" . $r9['user_id'] . " " . $condA . " group by a.user_name,a.page_id order by hits desc";
	$res91 = get_all($sql91);
	$hitsInfo = "";
...
echo "</tbody></table>";
$sql10 = "select a.user_id,a.user_name,count(*) as hits from visitors_info a inner join wp_posts b on b.ID=a.page_id " . $alphaCondA . $alphaXCondA . " " . $condA . " group by a.user_name order by hits desc";
$res10 = get_all($sql10);
