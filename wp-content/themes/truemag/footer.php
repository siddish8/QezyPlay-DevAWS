...
if(is_single()) //is a channel
{
	...
	$free_boq=$wpdb->get_var("SELECT is_free from bouquets where ID=(SELECT bouquet_id from bouquet_vs_channels where channel_id=".$post_id.")");

	if(  ($subStatus==1 or ((int)$subAgent>0 and $validity2==1)) or $user_access or $free_boq) 
	{
	$play=1;
...
/*
try{    
    $dbconAnalytics = new PDO('mysql:host=ideabytesdb.c6hujshgwzfd.us-east-1.rds.amazonaws.com;port=3306;dbname=cmadmin_qezyplay_analytics', 'cmadmin_dev', 'CmAdmin123', array( PDO::ATTR_PERSISTENT => false));
