...
{	
	...
	else
	{
		...
		$plan=$wpdb->get_var("select membership_id from wp_pmpro_memberships_users where user_id=".$this_user." and status='active' order by id desc limit 1");
		$txt="current plan:".$plan."\n";
		fwrite($file,$txt);
		...
		fwrite($file,$txt);
			//end
				...
				else
				{
					...
					if($plan==4){
						//if cuur plan is free-trial go for instant payment
						$ft_delay=$subscription_delay;
						update_user_meta( $this_user, "delay_from_free_trial", $ft_delay); 
						$subscription_delay=0; //make sub delay to be zero
						
						$start_date = date("Y-m-d", strtotime("+ " . intval($subscription_delay) . " Days", current_time('timestamp'))) . "T0:0:0";
						$txt="Delay Exists but curr plan is free trial so delay is now : $subscription_delay so start date is :".$start_date;
						fwrite($file,$txt);

					}
					else{
						//$ft_delay=0;
						//add_user_meta( $this_user, "delay_from_free_trial", $ft_delay);
						$start_date = date("Y-m-d", strtotime("+ " . intval($subscription_delay) . " Days", current_time('timestamp'))) . "T0:0:0";
