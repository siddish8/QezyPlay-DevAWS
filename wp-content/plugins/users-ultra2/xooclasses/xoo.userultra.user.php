			...
			if ( $type == 'usermeta' && $deleted == 0 && $private == 0)			
			 {
				...
				$html .= '<div class="xoouserultra-field-value">';
					...
					if($meta=='phone')
					{
						$countrycode=$this->get_user_meta("countryCode");
						$ph=$this->get_user_meta("ph");
						$phone=$this->get_user_meta( $meta);
					$html.=$this->get_countries_select();
					$html.='<input type="text" class="xoouserultra-input'.$required_class.'" 					name="'.$meta.'" id="'.$meta.'" value="'.$ph.'"  title="'.$name.'" onblur="return validate(this.id)" onclick="return styleback(this.id)" />
				<!-- </div><div id="phone_err_div" class="xoouserultra-field" style="display:none"> -->			
<input type="hidden" name="ph" value="" id="ph" value="'.$ph.'" /></div>
					<div id="phone_err_div" class="xoouserultra-field" style="display:none">					
   ...
   </div></div><div class="xoouserultra-clear"></div><script>jQuery("#countryCode").val("'.$countrycode.'")</script>';
   					if($ph=="" and $countrycode=="" and $phone!=""){
						   $html.='<script>jQuery("#phone_err_div").show();jQuery("#phone_err").html("Please change this '.$phone.' by selecting country and enter number")</script>';
					   }
					   $html.='<script>var phone='.$phone.';
					   jQuery("#phone1").val(phone);</script>';
					continue;}
...
<input type="submit" onclick="return validate(this.id)" name="xoouserultra-update" id="xoouserultra-update" class="xoouserultra-button" value="'.__('Update','xoousers').'" />
    ...
    outline: none;}</style><script>document.getElementById("phone_err_div").style.display="block";document.getElementById("phone_err").innerHTML="Please enter a contact no.";</script>';
							...
							/*elseif(!preg_match("/^[0-9]{5,11}$/", $value))
							{
					
								$_SESSION['errMsg'] .='<strong>ERROR:</strong> Please enter a valid contact no. | <style>#phone{border: 1px solid red ;  box-shadow: 0 0 10px red ;  outline: none;}</style><script>document.getElementById("phone_err_div").style.display="block !important";document.getElementById("phone_err").innerHTML="Please enter a valid contact no.";</script>'; 
							}*/
							else
							{
