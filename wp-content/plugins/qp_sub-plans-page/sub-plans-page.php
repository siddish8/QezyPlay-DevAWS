...
<span id="coupon_code_text">Enter your Coupon/Promo code:</span> <input onkeypress="return event.charCode !=32" onclick="return jQuery('#msg').html('');" type="text" id="cc" name="cc" /> <input type="hidden" id="app" name="app" value="0" /> <input type="submit" onclick="return chk_cc()" name="cc_submit" id="cc_submit" value="Submit" />
...
//Validate 1.Exist 2.Used
	...
	//$promo_cc_max_used=$wpdb->get_var("SELECT count(*) from promo_codes where usage_count<max_count and coupon_code='".$cpn."'");
		...
		if($app==1){
			...
			$msg[1]="<script>swal('Coupon Code expired')</script>";
		}
		...
		if($app==1){
			...
			if($got_freetrial>0){
					$msg[0]="3";
					$msg[1]="<script>swal('Oops! You have already used your free trial, you cannot use free trial ".$code_txt." code');
</script>";
					return $msg;
				}
			}	
			if($sub_active=="true")
				{
					...
					if($in_free_trial_plan>0 && $in_free_trial_enddate!="0000-00-00 00:00:00")
					{
						...
						$sub_stat="<p> Hurray! $cpn_plan_name  gift ".$code_txt." code is activated</p> <p>Your Subscription is extended from $prev_exp_date to $curr_exp_date </p>";
						$msg[0]="4";
						$msg[1]='<script>swal({   title: "",   text: "'.$sub_stat.'",   type: "warning",  html:"true",  showCancelButton: false,   confirmButtonColor: "#DD6B55",   confirmButtonText: "OK",   closeOnConfirm: false }, function(){   window.location.href="'.$link.'"; });</script>';
					...
					$msg[0]="5";
					$msg[1]="<script>swal('Oops! You are Already in subscription. Please share this Gift ".$code_txt." code to your friend/family')</script>";
					return $msg; 
					}
				}
			else
		...
		if($plan_id==4)
			...
			$sub_stat.="Hurray! $plan_name subscription plan activated .";
					...
					$msg[0]="6";
					$msg[1]='<script>swal({   title: "",   text: "'.$sub_stat.'",   type: "warning",   showCancelButton: false,   confirmButtonColor: "#DD6B55",   confirmButtonText: "OK",   closeOnConfirm: false }, function(){   window.location.href="'.$link.'"; });</script>';
	...
	elseif($promo_cc_exist>0)
	{
		...
		$promo_used_user=get_option("promo_used_".$user_id);
			...
			if($sub_active=="true")
				{
				 ...
				 $promo1= 'You can share this code with your Friends/Family';
					...
					if($app==1){
						...
						$msg[0]="4";
					$msg[1]='<script>swal({   title: "",   text: "'.$sub_stat.'",   type: "warning",  html:"true",  showCancelButton: false,   confirmButtonColor: "#DD6B55",   confirmButtonText: "OK",   closeOnConfirm: false }, function(){   window.location.href="'.$link.'"; });</script>';
					//$msg[1]='<script>swal({   title: "",   text: "'.$sub_stat.'",   type: "warning",  html:"true",  showCancelButton: false,   confirmButtonColor: "#DD6B55",   confirmButtonText: "OK",   closeOnConfirm: false }, function(){   window.location.href="m-subscription?user_id='.$user_id.'&user_name='.$username.'"; });</script>';	 						
