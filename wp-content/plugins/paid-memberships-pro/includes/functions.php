...
function pmpro_url($page = NULL, $querystring = "", $scheme = NULL)
{
	...
	if ((strpos($_SERVER['HTTP_USER_AGENT'], 'Mobile/') !== false) && (strpos($_SERVER['HTTP_USER_AGENT'], 'Safari/') == false) or $_SESSION['app']==1 or $_SERVER['HTTP_X_REQUESTED_WITH'] == "com.ideabytes.qezyplay.qezyplay_new") {
	//ios_app_user_agent_name //android_app_package_name
	//app but not detected via agent/package so fetch it from session variable set
    	$site_url=site_url();
	$url=str_replace($site_url."/", $site_url."/m-", $url);}

	// if( or $_SESSION['app']==1){ //android_app_package_name
	// $site_url=site_url();
	// $url=str_replace($site_url."/", $site_url."/m-", $url);}
...
$ft_delay=(int)get_user_meta($user_id,"delay_from_free_trial",true);
$txt.="ft_delayy:".$ft_delay;
if($old_planid==4 and $ft_delay>0){
	$delay=0;
	$plan_startdate=date('Y-m-d', strtotime($startdate. ' +'.(int)$delay.' days'));
	$txt.="new case of free_trial instant paymnt: with delay".$ft_delay;
