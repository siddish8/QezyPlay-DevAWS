...
function pmpro_shortcode_account($atts, $content=null, $code=""){
 ...
 //echo "pa:".$_SERVER['PHP_SELF'];
  //echo "app:".$_SESSION['app'];
	//echo "get1:".$_GET['user_name'];
// echo "server1:".$_SESSION['uid'];
 //echo "id1:".$_GET['user_id'];
 //echo "<script>console.log('sid::'".$_GET['user_id'].")</script>";
	...
	<div id="pmpro_account">	
		...
		$ft_delay=get_user_meta($this_user,"delay_from_free_trial",true);
		//echo "<script>console.log('Delay from FT:".$ft_delay."')</script>";
		if((int)$ft_delay>0){
				$ft_delay_txt="<br> **<sup>$ft_delay days will be added at the end of subscription</sup>";
		}else{
				$ft_delay_txt="";
		}
					...
					?>
						...
						if(!$cancelled) { ?> 
							...
							<td class="pmpro_account-membership-start">
								 <p><?php echo $planstartdate->format('d-m-Y');echo "<br />"; if($today < $planstartdate) {/*echo "(Days left in trial/carry: ".get_option('pmpro_sub_support_delay_' . $current_user->ID).")";*/ echo "(Days left in trial/carry:".$delay.")".$ft_delay_txt;}else {echo "Plan Started".$ft_delay_txt;}?></p> <?php } ?>
  							</td>
