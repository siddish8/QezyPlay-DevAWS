...
/*Process uploads*/
			...
			if ( $type == 'usermeta' && $deleted == 0 && $private == 0)			
			 {
				...
				$html .= '<div class="xoouserultra-field-value">';
					...
					$html.='<input type="text" class="xoouserultra-input'.$required_class.'" name="'.$meta.'" id="'.$meta.'" value="'.$ph.'"  title="'.$name.'" onblur="return validate(this.id)" onclick="return styleback(this.id)" /></div><!--<div id="phone_err_div" class="xoouserultra-field" style="display:none">-->
    ...
    <label id="phone_label" class="xoouserultra-field-type"></label><div id="phone_err" class="alert alert-danger xoouserultra-field-value"></div>
   					...
   					//hide country and label
					   $html.="<script>
					      jQuery('label[for=\"country\"]').hide();
						   jQuery('#country').hide();
						   jQuery('input#hide_country').hide();
						   jQuery('label[for=\"hide_country\"]').hide();
						   
							   </script>";
						 $html.="<script>
							   var countryfull=jQuery('#countryCode option:selected').text();
						   var country=countryfull.substring(countryfull.indexOf(' '));
						   country = countryfull.replace(country, '');						   
