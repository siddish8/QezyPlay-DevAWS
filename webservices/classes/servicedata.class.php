...
Class ServiceData extends DBCon
	...
	function processRegistration($username, $email, $password, $phone, $cc, $ip, $countryCode)
		{

			define("SITE_URL","http://qezy.tv");
		...
		try
			...
			$phone_cc="+".$countryCode.$phone;

			// $pass=md5($username.$userid);

			
			$sql22 = 'INSERT INTO wp_users(user_login,user_pass,user_email,user_nicename,phone,user_registered,display_name) VALUES("' . $username . '","' . $pass . '","' . $email . '","' . $username . '","' . $phone_cc . '","' . $date . '","' . $username . '")';
			...
			$meta_key = "countryCode";
			$sql24 = 'INSERT INTO wp_usermeta(user_id,meta_key,meta_value) VALUES("' . $userid . '","' . $meta_key . '","'.$countryCode.'")';
			$exec24 = $this->sql_execute($sql24);
			...
			$meta_key = "phone";
			$sql24 = 'INSERT INTO wp_usermeta(user_id,meta_key,meta_value) VALUES("' . $userid . '","' . $meta_key . '","'.$phone_cc.'")';
			$exec24 = $this->sql_execute($sql24);
...
//ended
	...
	function new_user_reg_cpn($user_id, $cc)
		...
		function processDeviceRegistration($user_id,$unique_id,$device_id,$app){
			$sql= 'SELECT count(*) from user_notify_devices where unique_id="'.$unique_id.'" and device_id="'.$device_id.'" and app="'.$app.'"';
			$device_already_registered=$this->get_var($sql);
			if((int)$device_already_registered==0){

				$sql0= 'SELECT count(*) from user_notify_devices where unique_id="'.$unique_id.'" and app="'.$app.'"';
				$device_registered=$this->get_var($sql0);
				if((int)$device_registered>0){
					$sql1 = 'UPDATE user_notify_devices SET device_id="'.$device_id.'" where unique_id="'.$unique_id.'" and app="'.$app.'"';
					$message="Device Updated";
				}else{
					$sql1 = 'INSERT into user_notify_devices(user_id,unique_id,device_id,app) VALUES(' . $user_id . ',"' . $unique_id . '","' . $device_id . '",' . $app . ')';
