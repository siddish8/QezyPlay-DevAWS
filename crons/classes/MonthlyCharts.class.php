...
class MonthlyCharts
{
	...
	function chartsmonthly()
	{
		...
		{   			
			$sql = "SELECT vi.city,vi.country_code,vi.country,count(vi.id) AS clicks FROM visitors_info as vi WHERE vi.geo_info_status = 1 AND date(datetime) BETWEEN '".$date1."' AND '".$date2."'".$friendlyIpsCond.$friendlyWebsites." GROUP BY vi.country ORDER BY clicks DESC";
		}
