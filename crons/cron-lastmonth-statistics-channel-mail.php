...
if(count($chans) > 0){
	...
	foreach($chans as $chan){		
		...
		$date_in_subject = date('F Y', strtotime('first day of last month')); //prev month
		//$date_in_subject = Date("F Y", strtotime($date_in_subject . " last month"));	//prev prev month

		$subject = "Qezyplay Analytics - ".$channel_name." | Monthly report | ".$date_in_subject." | Timezone:UTC/GMT";
		$url = SITE_URL.'/crons/reports/reports_lastmonth-channel.php?channel_id='.$channel_id.'&channel_name='.$channel_name;	
